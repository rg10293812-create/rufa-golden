...
def _image_candidates(html, base_url):
    ...
    for key in ['og:image','og:image:secure_url','twitter:image','image']:
        img=_meta_content(html, [key])
        if img: imgs.append(_abs_url(base_url,img))
    for pat in [r'"image"\s*:\s*"([^"]+)"', r'"images"\s*:\s*\[(.*?)\]', r'<img[^>]+src=["\']([^"\']+)["\']']:
        for m in re.finditer(pat, html, re.I|re.S):
            val=m.group(1)
            if pat.endswith('\](.*?)'):
                pass
            if ',' in val and 'http' in val:
                for x in re.findall(r'"([^"]+)"', val): imgs.append(_abs_url(base_url,x))
            else:
                imgs.append(_abs_url(base_url,val))
    clean=[]; seen=set()
    for img in imgs:
        if not img or img in seen: continue
        low=img.lower()
        if any(x in low for x in ['logo','icon','sprite','avatar']): continue
        if not (low.startswith('http://') or low.startswith('https://')): continue
        seen.add(img); clean.append(img)
        if len(clean)>=8: break
    return clean

def _download_images_as_data_urls(urls, max_count=6, max_each=1800000):
    saved=[]
    for img_url in urls[:max_count]:
        try:
            req=urllib.request.Request(img_url, headers={'User-Agent':'Mozilla/5.0 RufaGoldenBot/1.0'})
            with urllib.request.urlopen(req, timeout=8) as resp:
                ctype=(resp.headers.get_content_type() or 'image/jpeg')
                if not ctype.startswith('image/'): continue
                raw=resp.read(max_each+1)
                if not raw or len(raw)>max_each: continue
                saved.append(f"data:{ctype};base64,"+base64.b64encode(raw).decode('utf-8'))
        except Exception:
            continue
    return saved

def parse_property_link(url):
    """يجلب بيانات الإعلان والصور من الرابط ويجهزها لحقول النظام."""
    url=(url or '').strip()
    parsed=urllib.parse.urlparse(url)
    if parsed.scheme not in ['http','https'] or not parsed.netloc:
        raise ValueError('الرابط غير صحيح. انسخ رابط الإعلان كامل ويبدأ بـ http أو https')
    req=urllib.request.Request(url, headers={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 RufaGolden/1.0'})
    ...
    full=' '.join([title, desc, _clean_text(html[:250000])])
    price=''
    for pat in [r'([0-9][0-9,\.\s]{4,})\s*(?:ريال|ر\.س|SAR)', r'(?:ريال|ر\.س|SAR)\s*([0-9][0-9,\.\s]{4,})', r'"price"\s*:\s*"?([0-9][0-9,\.\s]{4,})"?']:
        m=re.search(pat, full, re.I)
        if m:
            digits=re.sub(r'[^0-9]', '', m.group(1))
            if digits:
                try: price=f'{int(digits):,} ريال'
                except Exception: price=digits
            break
    location=''
    for pat in [r'"addressLocality"\s*:\s*"([^"]+)"', r'"streetAddress"\s*:\s*"([^"]+)"', r'(?:في|حي)\s+([^\|\-،,]{3,45})']:
        m=re.search(pat, full, re.I)
        if m:
            location=_clean_text(m.group(1)); break
    specs=(desc or title or '').strip()
    if url not in specs:
        specs=(specs + '\n\nرابط الإعلان: ' + url).strip()
    image_urls=_image_candidates(html, url)
    downloaded=_download_images_as_data_urls(image_urls)
    images=downloaded or image_urls[:6]
    name=title[:180] if title else 'عقار من رابط إعلان'
    return {'name':name, 'location':location, 'price':price, 'specs':specs, 'source_url':url, 'images':images}
...
def properties():
    ...
    if request.method=='POST':
        ...
        if request.form.get('form_type')=='import_link':
            ...
            uploaded=[]
            imported_images=request.form.get('imported_images','')
            try:
                data=json.loads(imported_images or '[]')
                if isinstance(data,list): uploaded.extend([x for x in data if x])
            except Exception:
                pass
            ...
            row=Property(name=request.form.get('name',''), location=request.form.get('location',''), price=request.form.get('price',''), specs=request.form.get('specs',''), image_data=uploaded[0] if uploaded else '', images_data=json.dumps(uploaded[1:] if len(uploaded)>1 else [], ensure_ascii=False), source_url=request.form.get('source_url',''), status=request.form.get('status','available'), marketer_id=mid)
...
def property_detail(pid):
    ...
    return render_template('property_detail.html', row=row, images=row.images_list())
